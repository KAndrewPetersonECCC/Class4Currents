...
def superobb(file, near_method='simple'):
    ...
    time0 = time.time()
    if ( near_method == 'simple' ):
        func = lambda lon, lat : find_value_at_point.find_nearest_point_list(lon, lat, nav_lon, nav_lat)
    elif ( near_method ==  'gcircle' ):
        func = lambda lon, lat : find_value_at_point.find_nearest_gcircle_list(lon, lat, nav_lon, nav_lat)
    elif ( near_method ==  'fcircle' ):
        func = lambda lon, lat : find_value_at_point.find_nearest_gcircle_list(lon, lat, nav_lon, nav_lat, fast=True)
    IJPXS = xr.apply_ufunc(func, lon, lat, output_core_dims=[['NPTS']], dask="parallalized")
    print('NEAREST TIME', time.time() - time0)
    ...
    print('REARRANGE TIME', time.time() - time0)
    ...
    print('SUPEROBB TIME', time.time() - time0)

    if ( near_method == 'simple' ): outfile = file.replace('.nc', '_sobb.nc')
    if ( near_method == 'gcircle' ): outfile = file.replace('.nc', '_sobc.nc')
    if ( near_method == 'fcircle' ): outfile = file.replace('.nc', '_sobc.nc')
    SLA_NU.to_netcdf(outfile)
    return

def test(file=file):

    time0 = time.time()
    superobb(file, near_method='fcircle')
    print("TOTAL TIME GCIRCLE FAST", time.time() - time0)
    
    time0 = time.time()
    superobb(file, near_method='gcircle')
    print("TOTAL TIME GCIRCLE", time.time() - time0)

    time0 = time.time()
    superobb(file, near_method='simple')
    print("TOTAL TIME SIMPLE", time.time() - time0)
        
    return
       
def test1(file=file, method='simple'):

    time0 = time.time()
    superobb(file, near_method=method)
    if ( method == 'fcircle' ):
        print("TOTAL TIME GCIRCLE FAST", time.time() - time0)
    elif  ( method == 'gcircle' ):
        print("TOTAL TIME GCIRCLE", time.time() - time0)
